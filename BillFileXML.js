    ...
    if (typeof TheCall.Version3 != 'undefined') {
            //if (typeof TheCall.Version3.eRecord != 'undefined') {
                ...
                if (u.IsOk(obj["eRecord.01"]) == true) {
                    XML.WES("eRecord.01", obj["eRecord.01"]);                    
                };

                XML.WSE("eRecord.SoftwareApplicationGroup");

                if (u.IsOk(obj["eRecord.02"]) == true) {
                    XML.WES("eRecord.02", obj["eRecord.02"]);
                    
                };

                if (u.IsOk(obj["eRecord.03"]) == true) {
                    XML.WES("eRecord.03", obj["eRecord.03"]);                    
                };
                if (u.IsOk(obj["eRecord.04"]) == true) {
                    XML.WES("eRecord.04",  obj["eRecord.04"]);
            ...
            if (typeof TheCall.Route != 'undefined') {
                XML.WSE("EncouterRoute");
                for (var x = 0; x < TheCall.Route.length; x++)
                {
                    var enc = TheCall.Route[i].split("|");
                    var encTime = enc[0];
                    var encGeo = enc[1];
                    var encSpeed = enc[2];
                    ...
                    XML.WSE("EncouterCoord");
                    XML.WES("Timestamp", encTime);
                    XML.WES("Coords", encGeo);
                    XML.WES("Speed", encSpeed);
                    XML.WES("Battery", encBattery);
                    XML.WES("Altitutde", encAlt);
                    XML.WEE();
                }
                XML.WEE();
            };
